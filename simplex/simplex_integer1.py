...
Node = namedtuple("Node", ['a', 'cur_base_var'])
...
class simplex_integer:
    ...
    def simplex_calculate(self, a, cur_base_var, p, q):
    ...
    def simplex_transform(self, a, cur_base_var, method, phase=None):
        # lap lai bien doi don hinh den khi khong con phan tu tru
        if self._print:
            print('# 0: ',' base = ' , ['X' + str(i+1) for i in cur_base_var], '\n' + self.get_pretty_print_tableau(a, cur_base_var))
            print('======================================================================================================')
        it = 0
        while(True):
            if method == 'simplex': p, q = self.get_pivot(a, cur_base_var, phase)
            elif method == 'dual_simplex': 
                p, q = self.get_pivot_dual_simplex(a, cur_base_var)
            if p == -1 or q == -1: 
                if method == 'dual_simplex':
                    if p >= 0 and q == -1:
                        ...
                        return True
                    # Neu ton tai hang am ma khong co phan tu pivot am thi dual simplex vo nghiem
                elif method == 'simplex':
                    if q >= 0 and p == -1:
                        if self._print:
                        ...
                        return True
                    # neu ton tai cot duong ma khong co phan tu pivot duong thi simplex vo nghiem 
                break
            self.simplex_calculate(a, cur_base_var, p, q)
            it += 1
            if self._print:
    ...
    def solve_phase_1(self):
        ...
        if self._print:
            print('Begin phase 1', '\n')
        self.simplex_transform(self.a, self.cur_base_var, 'simplex', phase=1)
        M, N = self.A.shape
        if self.a[-1, -1] != 0:
            if self._print:
                ...
                if self.cur_base_var[i] in self.artificial_var:
                    ...
                    for index1, j in enumerate(self.a[i, 0:N]):
                        if index1 not in self.cur_base_var and j != 0:
                            ...
                            self.simplex_calculate(self.a, self.cur_base_var, i, index1)
                            removed = True
                            break
    ...
    def solve_phase_2(self):
        if self._print:
            print('Begin phase 2: ', '\n' )
        if self.simplex_transform(self.a, self.cur_base_var, 'simplex'):
            if self._print:
                print('End phase 2')
            return True
        else:
    ...
    def get_pivot(self, a, cur_base_var, phase=None):
    ...
    def get_pivot_dual_simplex(self, a, cur_base_var):
        #lay ra vi tri phan tu tru
        p = -1
        q = -1
        min_value = 0
        ...
        for index, value in enumerate(a[p, :-1]):
            if value < 0 and index not in cur_base_var :
                ...
                if ratio < min_value:
                    min_value = ratio
                    q = index
        # ti le am nho nhat lam cot, he so phai >= 0
        return p, q
    ...
    def solve_gomory(self):
        ...
        # for index, i in enumerate(self.cur_base_var):
            if self.cur_base_var[i] in self.int_set and self.a[:-1, -1][i].denominator != 1:
                ...
                if not self.simplex_transform(self.a, self.cur_base_var, 'dual_simplex'):
                    return False
                i = -1
            i += 1
        return True
    
    def branch_and_bound_cut(self, a, cur_base_var, row):
        if self._print:
            print('Adding Branch and bound cut on X[' + str(row+1) + "] = " + str(a[row, -1]))
        M, N = a.shape
        a_less = a.copy() # them rang buoc x <=
        a_great = a.copy() # them rang buoc x >=
        cur_base_var_less = cur_base_var.copy()
        cur_base_var_great = cur_base_var.copy()

        # x <= floor(x0) => x + x' = floor(x0)
        less_equal_constraint = [Fraction(0) for i in range(N)]
        less_equal_constraint[cur_base_var[row]] = Fraction(1)
        less_equal_constraint[-1] = Fraction(math.floor(a[row, -1]))
        cur_base_var_less.append(a_less.shape[1] - 1)

        # x >= floor(x0) => -x + x' = -floor(x0)
        great_equal_constraint = [Fraction(0) for i in range(N)]
        great_equal_constraint[cur_base_var[row]] = Fraction(-1)
        great_equal_constraint[-1] = -Fraction(math.ceil(a[row, -1]))
        cur_base_var_great.append(a_great.shape[1] - 1)
        
        new_varible = np.array([Fraction(0) for i in range(M+1)]).reshape(1, M+1)
        new_varible[:, -2] = Fraction(1)

        a_less = np.insert(a_less, M-1, [less_equal_constraint], axis=0)
        a_less = np.insert(a_less, N-1, new_varible, axis=1)

        a_great = np.insert(a_great, M-1, [great_equal_constraint], axis=0)
        a_great = np.insert(a_great, N-1, new_varible, axis=1)
        
        if self._print:
            print('Cut on x + x\' = floor(x0)')
            print('======================================================================================================')
            print(self.get_pretty_print_tableau(a_less, cur_base_var_less))
        # 1: bien doi don hinh 1 phat de dua cac bien co so ve dang dung
        # 2: bien doi dual simplex 
        self.simplex_calculate(a_less, cur_base_var_less, row, cur_base_var_less[row])
        result_less = self.simplex_transform(a_less, cur_base_var_less, 'dual_simplex') # result less ket qua co tim thay nghiem hay khong tai less

        if self._print:
            print('Cut on -x + x\' = -floor(x0)')
            print('======================================================================================================')
            print(self.get_pretty_print_tableau(a_great, cur_base_var_great))
        self.simplex_calculate(a_great, cur_base_var_great, row, cur_base_var_great[row])
        result_great = self.simplex_transform(a_great, cur_base_var_great, 'dual_simplex') # result great = ket qua co tim thay nghiem hay khong tai great
        return a_less, cur_base_var_less, result_less, a_great, cur_base_var_great, result_great
    ...
    def solve_branch_and_bound(self):
        ...
        i = 0
        pq = PriorityQueue()
        cur_solution = Node(self.a, self.cur_base_var)
        pq.push(cur_solution, self.a[-1, -1])
        while(not pq.empty()):
            found = True
            value, _, solution = pq.pop()
            cur_base_var = solution.cur_base_var
            a = solution.a
            if self._print:
                print('POP FROM QUEUE VALUE = ', value)
                print('======================================================================================================')
                print('base = ' , ['X' + str(i+1) for i in cur_base_var], '\n' + self.get_pretty_print_tableau(a, cur_base_var))
            for i in range(len(cur_base_var)):
                if cur_base_var[i] in self.int_set and a[:-1, -1][i].denominator != 1:
                    # tim duoc 1 nghiem khong nguyen -> branch and cut
                    a_less, cur_base_var_less, result_less, a_great, cur_base_var_great, result_great = \
                        self.branch_and_bound_cut(a, cur_base_var, i)
                    if result_less:  pq.push(Node(a_less , cur_base_var_less ), a_less[-1, -1])
                    if result_great: pq.push(Node(a_great, cur_base_var_great), a_great[-1, -1])
                    found = False
                    break
            # tat ca cac nguyen deu nguyen -> thoat
            if found:
                if self._print:
                    print('All roots are integer, exitting')
                self.a = a
                self.cur_base_var = cur_base_var
                return True
        return False
    ...
    def get_pretty_print_tableau(self, a, cur_base_var):
        a1 = a.copy()
        b1 = ['X'+ str(x+1) + '  |' for x in cur_base_var]
        b1 = np.append(b1, [' ' for i in range(a1.shape[0] - len(cur_base_var))])
